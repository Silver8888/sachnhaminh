...
CREATE TABLE IF NOT EXISTS public.site_settings (
    ...
    partners_animation text DEFAULT 'left',
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
-- Bảng Đối tác Thân thiết (Partners)
...
-- Kích hoạt RLS cho partners
ALTER TABLE public.partners ENABLE ROW LEVEL SECURITY;

-- Chính sách đọc partners công khai
DROP POLICY IF EXISTS "Cho phép đọc partners công khai" ON public.partners;
CREATE POLICY "Cho phép đọc partners công khai" ON public.partners FOR SELECT USING (true);

-- Chính sách chỉnh sửa partners chỉ dành cho Admin
DROP POLICY IF EXISTS "Chỉ Admin được quản lý partners" ON public.partners;
CREATE POLICY "Chỉ Admin được quản lý partners" ON public.partners FOR ALL USING (public.is_admin());
